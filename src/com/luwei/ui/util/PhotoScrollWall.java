...
import com.luwei.activity.ImageDetailsActivity;
import com.luwei.domain.Images;
import com.luwei.network.ImageWallData;
 ...
 * @author guolin
 */
public class PhotoScrollWall extends ScrollView implements OnTouchListener {
	...
	private static Handler handler = new Handler() {
		PhotoScrollWall myScrollView;

		@Override
		public void handleMessage(android.os.Message msg) {
			if (msg.what == TOUCH) {
				myScrollView = (PhotoScrollWall) msg.obj;
			...
			if (msg.what == GETURL) {
				myScrollView = (PhotoScrollWall) msg.obj;
				int i = 0;
				urls= new String[myScrollView.images.size()];
				for (Images image : myScrollView.images) {
					urls[i] = image.getUrl();
					i++;
				}
	 ...
	 * @param attrs
	 */
	public PhotoScrollWall(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
			...
			loadOnce = true;
			if (images == null) {
				ImageWallData.executeDataLoad(handler, this, 0, 20);
			} else {
				loadMoreImages();
			}
		}
	...
	/**
	 * 这是加载的起始点，所以在这之前需要获得URL 开始加载下一页的图片，每张图片都会开启一个异步线程去下载。
	 */
	...
	class LoadImageTask extends AsyncTask<Integer, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Integer... params) {
			...
			mImageUrl = images.get(mItemPosition).getUrl();
