 ...
 * associated request.
 */
public class RectNetworkImageView extends ImageView {
	...
	public RectNetworkImageView(Context context, AttributeSet attrs) {
	 ...
	 * 
	 * NOTE: If applicable,
	 * {@link CircledNetworkImageView#setDefaultImageResId(int)} and
	 * {@link CircledNetworkImageView#setErrorImageResId(int)} should be called
	...
	/**
	 * 以下method就是扩展的 亮点是作者使用了reflect，获取bitmap
	 */
	/**
	 * In case the bitmap is manually changed, we make sure to circle it on the
	 * next onDraw
	 */
	@Override
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!mCircled && getDrawable() != null) {
			...
			try {
				...
				Bitmap bitmap = (Bitmap) d.getClass().getMethod("getBitmap")
						.invoke(d);
				if (bitmap != null) {
					Bitmap rectBitmap = getRectBitmap(bitmap);
					setImageBitmap(rectBitmap);
				}
	...
	public static Bitmap getRectBitmap(Bitmap bitmap) {
		...
		Bitmap output = Bitmap.createBitmap(size, // 重画一张bitmap依据矩形
				size, Bitmap.Config.ARGB_8888);
		Canvas canvas = new Canvas(output);
		BitmapShader shader;						//将原图夹住，以此画形状
...
//		canvas.drawText("画圆角矩形:", 10, 30, paint);
		RectF oval3 = new RectF(0, 0, size, size);// 设置个新的长方形
		canvas.drawRoundRect(oval3, 10, 10, paint);// 第二个参数是x半径，第三个参数是y半径
		return output;
	}
